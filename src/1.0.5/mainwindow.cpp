...
#include <QTranslator>  // 确保包含这个头文件
#include <QApplication> // 确保包含这个头文件
    ...
    , systemWSLabel(nullptr)
{
    ...
    QAction *actionAuthor = new QAction(tr("作者主页"), this);
    ...
    QAction *russianAction = languageMenu->addAction("Русский");  // 添加俄语选项
...
void MainWindow::createTrayIcon() {
    ...
    // 创建二级菜单（清理选项）
    cleanSubMenu = new QMenu(tr("Clean Memory Areas"), this);

    // 添加清理选项到二级菜单
    trayWorkingSetAction = cleanSubMenu->addAction(tr("Working Set"));
    trayWorkingSetAction->setCheckable(true);

    // 修改为使用 tr() 函数
    traySystemWSAction = cleanSubMenu->addAction(tr("System WS"));
    traySystemWSAction->setCheckable(true);

    // 修改为使用 tr() 函数
    trayStandbyListAction = cleanSubMenu->addAction(tr("Standby"));
    trayStandbyListAction->setCheckable(true);

    // 修改为使用 tr() 函数
    trayStandbyListLowAction = cleanSubMenu->addAction(tr("Standby (Low Priority)"));
    trayStandbyListLowAction->setCheckable(true);

    // 修改为使用 tr() 函数
    trayModifiedListAction = cleanSubMenu->addAction(tr("Modified Page List"));
    trayModifiedListAction->setCheckable(true);

    // 修改为使用 tr() 函数
    trayCombinedListAction = cleanSubMenu->addAction(tr("Combine Page List"));
    ...
    // 2. 作者选项
    authorAction = new QAction(tr("Author"), this);
...
void MainWindow::saveSettings()
{
    ...
    if (!QFileInfo(QCoreApplication::applicationDirPath()).isWritable()) {
        QMessageBox::warning(this, "警告", "程序目录不可写，无法保存设置！");
        return;
    }

    QSettings settings(iniPath, QSettings::IniFormat);
    settings.setValue("language/current", currentLanguage); // 保存当前语言
    // 清理选项
...
void MainWindow::onTrayCleanWorkingSet()
{
    if (MemoryCleaner::cleanWorkingSet()) {
        ...
        trayIcon->showMessage(tr("Memory Cleaner"), tr("Working set cleaned successfully"), icon, 2000);
    } else {
        QSystemTrayIcon::MessageIcon icon = QSystemTrayIcon::MessageIcon(QSystemTrayIcon::Critical);
        trayIcon->showMessage(tr("Memory Cleaner"), tr("Failed to clean working set"), icon, 2000);
    }
}
...
void MainWindow::onTrayCleanSystemWS()
{
    if (MemoryCleaner::cleanSystemWorkingSet()) {
        QSystemTrayIcon::MessageIcon icon = QSystemTrayIcon::MessageIcon(QSystemTrayIcon::Information);
        trayIcon->showMessage(tr("Memory Cleaner"), tr("System working set cleaned successfully"), icon, 2000);
    } else {
        QSystemTrayIcon::MessageIcon icon = QSystemTrayIcon::MessageIcon(QSystemTrayIcon::Critical);
        trayIcon->showMessage(tr("Memory Cleaner"), tr("Failed to clean system working set"), icon, 2000);
    }
}
...
void MainWindow::showAboutFromTray()
{
    AboutDialog aboutDialog(this);
    aboutDialog.exec();
    trayIcon->showMessage(tr("About"), tr("Showing about dialog"), QSystemTrayIcon::Information, 1000);
}
...
// 修改后的 syncTrayMenuWithSettings 函数
void MainWindow::syncTrayMenuWithSettings() {
    QString iniPath = QCoreApplication::applicationDirPath() + "/settings.ini";
    QSettings settings(iniPath, QSettings::IniFormat);

    // 从 settings.ini 读取清理选项状态
...
void MainWindow::changeLanguage(const QString& locale) {
    ...
    currentLanguage = locale;  // 修改这里，使用参数locale而不是未声明的languageCode
    // 重翻译UI
    retranslateUi();  // 确保调用这个函数
}


void MainWindow::retranslateUi()
{
    ui->retranslateUi(this);  // 调用UI自动生成的翻译函数

    if (trayMenu) {
        cleanSubMenu->setTitle(tr("Clean Memory Areas"));
        trayWorkingSetAction->setText(tr("Working Set"));
        traySystemWSAction->setText(tr("System Working Set"));
        trayStandbyListAction->setText(tr("Standby List"));
        trayStandbyListLowAction->setText(tr("Standby List (Low Priority)"));
        trayModifiedListAction->setText(tr("Modified Page List"));
        trayCombinedListAction->setText(tr("Combined Page List"));
