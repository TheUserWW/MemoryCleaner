    ...
    , enableMessage(true)
{
    ...
    connect(ui->actionTraditionalChinese, &QAction::triggered, this, &MainWindow::onTraditionalChineseTriggered);
    connect(ui->actionMongolian, &QAction::triggered, this, &MainWindow::onMongolianTriggered);
    connect(ui->actionUyghur, &QAction::triggered, this, &MainWindow::onUyghurTriggered);
    connect(ui->actionEnglish, &QAction::triggered, this, &MainWindow::onEnglishTriggered);
    connect(ui->actionFrench, &QAction::triggered, this, &MainWindow::onFrenchTriggered);
    connect(ui->actionRussian, &QAction::triggered, this, &MainWindow::onRussianTriggered);
    connect(ui->actionSpanish, &QAction::triggered, this, &MainWindow::onSpanishTriggered);
    connect(ui->actionGerman, &QAction::triggered, this, &MainWindow::onGermanTriggered);
    connect(ui->actionItalian, &QAction::triggered, this, &MainWindow::onItalianTriggered);
    connect(ui->actionArabic, &QAction::triggered, this, &MainWindow::onArabicTriggered);
    connect(ui->actionKorean, &QAction::triggered, this, &MainWindow::onKoreanTriggered);
    connect(ui->actionJapanese, &QAction::triggered, this, &MainWindow::onJapaneseTriggered);
    connect(ui->actionAutoStart, &QAction::triggered, this, &MainWindow::onAutoStartTriggered);
    connect(ui->actionEnableMessage, &QAction::triggered, this, &MainWindow::onEnableMessageTriggered);
...
void MainWindow::loadSettings()
{
    ...
    bool autoStart = settings.value("autoStart/enabled", false).toBool();
    enableMessage = settings.value("enableMessage", true).toBool();
    
    ui->actionAutoStart->setChecked(autoStart);
    ui->actionEnableMessage->setChecked(enableMessage);
...
void MainWindow::saveSettings()
{
    ...
    if (!QFileInfo(QCoreApplication::applicationDirPath()).isWritable()) {
        QMessageBox::warning(this, "MemoryCleaner", tr("程序目录不可写，无法保存设置！\n请以管理员权限运行程序。"));
        return;
    }
    ...
    settings.setValue("autoStart/enabled", ui->actionAutoStart->isChecked());
    settings.setValue("enableMessage", enableMessage);
}
...
void MainWindow::updateMemoryInfo()
{
    ...
    else if(currentLanguage == "zh_TW") langIndex = 10;
    else if(currentLanguage == "ug_UG") langIndex = 11;
    else if(currentLanguage == "mn_CN") langIndex = 12;
...
void MainWindow::onTrayCleanWorkingSet()
{
    if (MemoryCleaner::cleanWorkingSet()) {
        ...
        if (enableMessage) {
            trayIcon->showMessage("MemoryCleaner", tr("工作集清理成功"), icon, 2000);
        }
    } else {
        QSystemTrayIcon::MessageIcon icon = QSystemTrayIcon::MessageIcon(QSystemTrayIcon::Critical);
        if (enableMessage) {
            trayIcon->showMessage("MemoryCleaner", tr("清理工作集失败"), icon, 2000);
        }
    }
}
...
void MainWindow::onChineseTriggered() { onLanguageChanged("zh_CN"); }
void MainWindow::onTraditionalChineseTriggered() { onLanguageChanged("zh_TW"); }
void MainWindow::onMongolianTriggered() { onLanguageChanged("mn_CN"); }
void MainWindow::onUyghurTriggered() { onLanguageChanged("ug_UG"); }
void MainWindow::onEnglishTriggered() { onLanguageChanged("en_US"); }
void MainWindow::onFrenchTriggered() { onLanguageChanged("fr_FR"); }
void MainWindow::onRussianTriggered() { onLanguageChanged("ru_RU"); }
void MainWindow::onSpanishTriggered() { onLanguageChanged("es_ES"); }
void MainWindow::onGermanTriggered() { onLanguageChanged("de_DE"); }
void MainWindow::onItalianTriggered() { onLanguageChanged("it_IT"); }
void MainWindow::onArabicTriggered() { onLanguageChanged("ar_AE"); }
void MainWindow::onKoreanTriggered() { onLanguageChanged("ko_KR"); }
void MainWindow::onJapaneseTriggered() { onLanguageChanged("ja_JP"); }
...
bool MainWindow::isAutoStartEnabled()
{
    ...
    if (RegOpenKeyExW(HKEY_CURRENT_USER, keyPath, 0, KEY_READ, &hKey) == ERROR_SUCCESS) {
        DWORD dataSize = 0;
        if (RegQueryValueExW(hKey, appName, NULL, NULL, NULL, &dataSize) == ERROR_SUCCESS) {
            RegCloseKey(hKey);
            return true;
        }
        RegCloseKey(hKey);
...
void MainWindow::onAutoStartTriggered()
{
    bool enabled = ui->actionAutoStart->isChecked();
    ...
    if (enableMessage) {
        QString message = enabled ? tr("已开启开机自启动") : tr("已关闭开机自启动");
        trayIcon->showMessage("MemoryCleaner", message, QSystemTrayIcon::Information, 2000);
    }
}

void MainWindow::onEnableMessageTriggered()
{
